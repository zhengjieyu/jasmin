...
Section PROOF.
  ...
  Lemma size_128_512_ge sz : (U128 <= sz)%CMP -> size_128_512 sz.
  Proof. by move=> h; rewrite /size_128_512 h wsize_ge_U512. Qed.
  ...
  Proof.
    ...
    + case: x => - [] [] [] // sz vn vi vs //= /[dup] ok_v.
      ...
      case: ifP.
      *move => h.
      case: ifP => sourceis_regmaskdestinationis_memory.

      have sz''_le_U64: (sz'' ≤ U64)%CMP.
      {
        exact: (cmp_le_trans hle (cmp_le_trans Hs' h)).
      }
        
        rewrite /= ok_v /exec_sopn /sopn_sem /= /size_8_64 sz''_le_U64
        /sopn_sem_ /= /x86_KMOVALL. 
        by rewrite ok_w /= zero_extend_u.
        case: ifP => destinationis_regmask.
        case: ifP => loadmaskisvalid; last done.  
        have sz''eqU64 : (sz'' == U64).
        { 
          apply andb_prop in loadmaskisvalid.
          destruct loadmaskisvalid as [_ HszU64].
          exact HszU64.
        }
        have sz''eqsz : (sz'' == sz).
        { 
          apply andb_prop in loadmaskisvalid.
          destruct loadmaskisvalid as [Hsz _].
          exact Hsz.
        }
        have /eqP Heq1 := sz''eqsz. 
        subst.
        rewrite /= ok_v /exec_sopn /sopn_sem /= /size_8_64 h  /sopn_sem_ /= /x86_KMOVALL /kmov_returnsz.
        rewrite ok_w /=.
        have /eqP Heq2 := sz''eqU64. 
        rewrite -Heq2.
        ...
        move => is_regmask. 
        case: ifP => storemaskisvalid; last done.
        rewrite /= ok_v /exec_sopn /sopn_sem /= /size_8_64 (cmp_le_trans hle (cmp_le_trans Hs' h)) /sopn_sem_ /= /x86_KMOVALL /kmov_returnsz. 
        rewrite ok_w /=.
        rewrite cmp_max_r.
        by rewrite zero_extend_u.
        exact: storemaskisvalid.
        ...
        * by rewrite /= ok_v /exec_sopn /sopn_sem /sopn_sem_ /= ok_w /x86_VMOVDQ /size_128_512 h128_le_sz'' wsize_ge_U512.
        ...
        rewrite /= ok_v /exec_sopn /sopn_sem /= /x86_KMOVALL /size_8_64 h /sopn_sem_.
        rewrite /x86_KMOVALL /=.
        rewrite /x86_KMOVALL /=.
        have /eqP Heq1 := movemaskisvalid. 
        subst sz.
        rewrite ok_w /=.
        ...
        have sz_eq_sz' : sz == sz'.
        {
          ...
          have /eqP Heq1 := movemaskisvalid. 
          subst sz''. 
          rewrite (cmp_le_antisym Hs' hle).
          exact: movemaskisvalid.
        }
        rewrite /x86_KMOVALL /=.
        have /eqP Heq1 := movemaskisvalid. 
        rewrite Heq1.
      ...
      * eexists; first reflexivity.
      ...
      rewrite (truncate_word_le _ hle) /=.
      move /truncate_valI: Hv'. => s'' [] w'' [] ? ok_w ?; subst.
    ...
    + case: o => // [[] sz |[] sz|[] sz|u []// sz| u []// sz|sz|sz|sz|sz|sz|sz|sz|sz| ve sz | ve sz | ve sz | ve sz | ve sz | ve sz] //.
      ...
      + case handn : is_andn => [[a1 a2] | ].
        + move=> he.
          ...
          case : eqP => //= ?; subst ty.
          rewrite /exec_sopn /sopn_sem /sopn_sem_ /= ha1 /= ha2 /= hva1 /= hva2 /=.
          rewrite /x86_VPANDN (wsize_nle_u64_size_128_512 hty) /=.
        ...
        * (* AND *)
        ...
        (* VPAND *)
        rewrite /x86_VPAND /=.
        by rewrite (wsize_nle_u64_size_128_512 hty) /=.
      (* Olor Op_w *)
      ...
      + case: ifP => // /andP [hle /eqP ?]; subst ty.
        ...
        by rewrite (size_128_512_ge hle) /= !truncate_word_le.
      (* Ovsub ve sz *)
      + case: ifP => // /andP [hle /eqP ?]; subst ty.
        rewrite /= /sem_sop2 /exec_sopn /sopn_sem /sopn_sem_ /=;t_xrbindP => v1 ok_v1 v2 ok_v2.
        move => ? /to_wordI' [sz1] [w1] [hw1 ??]; subst.
      ...
      + case: ifP => // /andP [/andP[hle1 hle2] /eqP ?]; subst ty.
        ...
        rewrite /size_16_32 hle1 (size_128_512_ge hle2).
      ...
      + case: ifP => // /andP [/andP [hle1 hle2] /eqP ?]; subst ty.
        ...
        rewrite (size_128_512_ge hle2) (size_16_64_ve hle1) /=.
        by rewrite !truncate_word_le.
      (* Ovlsl ve sz *)
      + case: ifP => // /andP [/andP [hle1 hle2] /eqP ?]; subst ty.
        rewrite /= /sem_sop2 /exec_sopn /sopn_sem /sopn_sem_ /=;t_xrbindP => v1 ok_v1 v2 ok_v2.
      ...
      + case: ifP => // /andP [/andP [hle1 hle2] /eqP ?]; subst ty.
        ...
        rewrite (size_128_512_ge hle2) hle1 /=.
        by rewrite !truncate_word_le.
    (* PappN *)
    + case: op => // - [] // - [] //.
      case: es => // - [] // [] // [] // [] // hi.
