...
module ATTSyntax : X86AsmSyntax = struct
  ...
  let pp_adress _ws (addr : (_, _, _, _, _, _) Arch_decl.address) =
    match addr with
...
module IntelSyntax : X86AsmSyntax = struct
  ...
  let pp_adress ws (addr : (_, _, _, _, _, _) Arch_decl.address) =
    match addr with
...
module X86AsmTranslate (AsmSyntax: X86AsmSyntax) = struct
  ...
  let asm_instr_r name (instr_r : (_, _, _, _, _, _, _) Arch_decl.asm_i_r) =
    match instr_r with
    | ALIGN ->
      [Instr (".p2align", ["5"])]
    | LABEL (_, lbl) ->
