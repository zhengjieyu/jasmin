...
let tt_reg_kind annot =
  ...
  | Some () -> W.Extra
  | None    -> 
    match Annot.ensure_uniq1 "regmask" Annot.none annot with
    | Some () -> W.Mask
    | None    -> W.Normal
...
let mk_op_s_k_info eop =
  ...
  ; opi_wcmp =  {intok = true; wintok = true}, cmp_8_512
  ; opi_vcmp = None }
...
let mk_logic_info eop =
  { opi_op = mk_logic_ws eop;
    opi_wcmp = {intok = false; wintok = false}, cmp_8_512;
    opi_vcmp = Some (cmp_8_64); }
...
type ('a, 'b, 'c, 'd, 'e, 'f, 'g, 'h) arch_info = {
  pd : Wsize.wsize;
  asmOp : ('a, 'b, 'c, 'd, 'e, 'f, 'g, 'h) Arch_extra.extended_op Sopn.sopn Sopn.asmOp;
